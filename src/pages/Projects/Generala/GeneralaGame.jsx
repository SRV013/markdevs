...
export const GeneralaGame = ({
    ...
    const renderCategoryData = (cat) => {
        const scoreValue = activePlayer.scores[cat.id];
        const isScored = scoreValue !== undefined;
        const isCrossedOut = scoreValue === 0;

        return (
            <div
                key={cat.id}
                className={`${styles.categoryInputRow} ${isScored ? styles.scoredOpacity : ''}`}
            >
                <div className={styles.catLeft}>
                    <span className={`${styles.catInputName} ${isCrossedOut ? styles.crossedText : ''}`}>{cat.name}</span>
                </div>
                <div className={styles.catRight}>
                    {isScored ? (
                        <div className={styles.scoredData}>
                            <span className={styles.scoreResult}>
                                {isCrossedOut ? 'Tachado' : `${scoreValue}`}
                            </span>
                            <button
                                className={styles.modifyBtn}
                                onClick={() => handleModifyScore(activePlayer.id, cat.id)}
                            >
                                Modificar
                            </button>
                        </div>
                        ...
                        isViewingOtherPlayer ? (
                            ...
                            <div className={styles.actionGroup}>
                                <select
                                    ...
                                    value=""
                                    onChange={(e) => handleSaveScore(activePlayer.id, cat.id, e.target.value)}
                                >
                                    <option value="" disabled>--</option>
                                    {cat.options.map(opt => (
                                        <option key={opt} value={opt}>
                                            {opt === 0 ? '0 o Tachar' : opt}
                                        </option>
                                    ))}
                                </select>
                            </div>
                ...
                <div className={styles.controlsCol}>
                    ...
                    <div className={styles.categoriesGrid}>
                        <div className={styles.categoriesGridCol}>
                            {leftCategories.map(cat => renderCategoryData(cat))}
                        </div>
                        <div className={styles.categoriesGridCol}>
                            {rightCategories.map(cat => renderCategoryData(cat))}
                        </div>
                    </div>
                </div>
                    ...
                    <div className={styles.turnCardWrapper}>
                        ...
                        <div className={styles.turnContent}>
                            <div className={styles.turnName}>
                                <span className={styles.activePlayerName}>{playerWithTurn?.name} - Te toca</span>
                            </div>
